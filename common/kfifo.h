 ...
 * to lock the reader.
 */
//#include <linux/kernel.h> //#include <linux/spinlock.h> //#include <linux/stddef.h> //#include <linux/scatterlist.h>
#include <pthread.h>
typedef pthread_spinlock_t spinlock_t;
#define spin_lock_irqsave(lock_ptr, flags) pthread_spin_lock(lock_ptr)
#define spin_unlock_irqrestore(lock_ptr, flags) pthread_spin_unlock(lock_ptr)
#define ARRAY_SIZE(x) (sizeof(x) / sizeof(*(x)))
struct __kfifo {
	unsigned int	in;
	unsigned int	out;
	unsigned int	mask; //size; mask*esize if kfifo_init
	unsigned int	esize; //the size of element
	void		*data;
};
...
//extern int __kfifo_from_user(struct __kfifo *fifo,const void *from, unsigned long len, unsigned int *copied);
//
